...
Styles (see STYLES at the bottom):
  banner     - solid block-colour band across the top (tag + title), figure below
  dark       - charcoal background, white title, figure on a rounded white card
...
Runtime: ~30 s for the default 4 lessons x 4 styles (measured value is logged).
...
def draw_title(fig, lines, size, x0, y_top, color, **extra):
    """Draw wrapped lines top-down; returns the y of the block's bottom."""
...
def style_banner(fig, lesson, img):
    """Replaced a left-panel "split" layout on 2026-09-26: the course figures are
    wide, so beside a panel they shrank to a strip. A full-width band keeps the
    bold colour and gives the figure the whole width."""
    color = block_color(lesson)
    ink = text_on(color)
    fig.text(0.05, 0.95, lesson["tag"], color=ink, fontsize=36, va="top",
             fontname=base.TAG_FONT)
    if lesson.get("practical"):
        pill(fig, 0.965, 0.915, "white", ink=color if color != ORANGE else CHARCOAL, ha="right")
    size, lines = fit_title(fig, lesson, max_w=0.90, max_lines=2, max_size=74, min_size=40)
    bottom = draw_title(fig, lines, size, 0.05, 0.84, ink)
    band_bottom = bottom - 0.035
    fig.add_artist(Rectangle((0, band_bottom), 1, 1 - band_bottom, transform=fig.transFigure,
                             facecolor=color, edgecolor="none", zorder=-1))
    place(fig, img, (0.04, 0.03, 0.92, band_bottom - 0.06))


def style_dark(fig, lesson, img):
    color = block_color(lesson)
    fig.patch.set_facecolor("#121620")
    fig.text(0.05, 0.95, lesson["tag"], color=ORANGE, fontsize=36, va="top",
             fontname=base.TAG_FONT)
    if lesson.get("practical"):
        pill(fig, 0.965, 0.915, color, ha="right")
    size, lines = fit_title(fig, lesson, max_w=0.90, max_lines=2, max_size=74, min_size=40)
    bottom = draw_title(fig, lines, size, 0.05, 0.84, "white")
    fig.add_artist(Rectangle((0.05, bottom - 0.005), 0.10, 0.012, transform=fig.transFigure,
                             facecolor=color, edgecolor="none"))
    pad_x = 0.018
    x, y, w, h = place(fig, img, (0.05 + pad_x, 0.04 + pad_x / K,
                                  0.90 - 2 * pad_x, bottom - 0.08 - 2 * pad_x / K))
    fig.add_artist(FancyBboxPatch(
        (x - pad_x, y - pad_x / K), w + 2 * pad_x, h + 2 * pad_x / K,
        boxstyle="round,pad=0,rounding_size=0.015", mutation_aspect=1 / K,
...
STYLES = {"banner": style_banner, "dark": style_dark,
          "bignumber": style_bignumber, "notebook": style_notebook}
