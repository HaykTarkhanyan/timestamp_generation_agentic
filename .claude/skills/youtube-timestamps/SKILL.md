...
**Per-lesson elements:**
- **Materials URL** — the GitHub Pages link to the lesson notes. **Resolve it automatically** from the course repo instead of leaving a placeholder: run
  ```bash
  python .claude/skills/youtube-timestamps/scripts/resolve_materials_url.py --lesson <NN>
  ```
  and paste the printed URL into the `📚 Նյութը՝` line. The published notes mirror the repo: a lecture's `<NN>_*.tex` lives in a section folder (e.g. `ml/03_classification/13_threshold_tuning.tex`), and that folder's chapter page is the materials link — so a whole section shares one page (lessons 11-16 → `.../ml/03_classification/03_classification.html`, lesson 17 → `.../ml/04_trees/04_trees.html`). Practicals have no own numbered `.tex`, so the script maps them to their chapter via the nearest lower-numbered lecture (lesson 16 → 03_classification). The course repo is a sibling of this project, NOT inside it: `C:\Users\hayk_\OneDrive\Desktop\01_python_math_ml_course` (it also holds datasets and class figures, useful for thumbnails). Only if the script fails loudly (unknown lesson, repo moved) fall back to the literal `TODO: paste materials URL here` (NO angle brackets — YouTube strips them) so the user spots it before posting. To backfill already-written descriptions, `resolve_materials_url.py --all` fills every `output/*/description.txt` whose materials line is still a TODO.
- **Model attribution** — the LLM that generated each section, in parentheses. The user wants honest attribution per section because timestamps and abstract may be generated by different models / different runs. Default to the model actually running the stage — as of 2026-09-26 that is `(Opus 5.5)` (from ML46 on; ML42-45 really were Opus 5, so their tags stay). Check which model you are before writing this line; do NOT copy the tag from an older lesson's description. If the same model generated both sections, put the same tag on both; if different, write each section's actual generator.
...
**YouTube caps a description at 5000 characters**, and `yt_publish.py set-description` refuses anything longer (after stripping the title header). Count before publishing: `LC_ALL=C.UTF-8 wc -m description.txt` (the header adds ~110). A normal lecture lands near 4200, but a long one with dense nested chapters overflows. When it does, cut in this order: fold minor sub-chapters into their parent (plumbing steps like "logging", "output layer", "the optimizer line"), shorten long labels, then trim the abstract back to 3-5 sentences. Re-run the verifier after cutting.

> Learned 2026-09-26: ML46 (1h40m practical) came out at 73 chapters and a ~1700-char abstract, 5788 chars in total, and the push failed. Dropping 7 plumbing sub-chapters, shortening labels and halving the abstract got it to 4450 with 66 chapters, all parsed by YouTube.
